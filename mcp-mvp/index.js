...
import { createThread, sendMessage, createDynamicAssistant } from './client/agents/assistant.js';
import { getJiraIssueTool, getJiraIssueExecutor, getDetailedJiraIssueTool, getDetailedJiraIssueExecutor, getJiraIssueCommentsTool, getJiraIssueCommentsExecutor, getJiraIssueTransitionsTool, getJiraIssueTransitionsExecutor, searchJiraIssuesTool, searchJiraIssuesExecutor, getJiraIssueWatchersTool, getJiraIssueWatchersExecutor, getJiraIssueAttachmentsTool, getJiraIssueAttachmentsExecutor, getJiraIssueSprintsTool, getJiraIssueSprintsExecutor, addJiraCommentTool, addJiraCommentExecutor } from './jiraTool.js';
...
server.resource("note", "note:///{noteId}", async (uri) => {
...
server.tool(getJiraIssueTool.name, {
...
async function callClaudeHaiku(messages, tools, sessionIdentifier) {
    const body = {
...
app.post('/chat', (async (req, res) => {
    ...
    if (!USE_ENV_FOR_JIRA_CREDENTIALS) {
        const userJiraCreds = req.body.jiraAuth;
    ...
    try {
        ...
        if (enableContextFiltering) {
            ...
            if (contexts.length > 0) {
                ...
                const toolsText = allTools.tools.map((t) => `- ${t.name}: ${t.description}`).join('\n');
                ...
                try {
                    if (selectedModel === 'openai') {
                        ...
                        const messages = [{ role: 'user', content: [{ type: 'text', text: promptText }] }];
                ...
                let suggestedToolNames = [];
                if (llmResponse && typeof llmResponse === 'string') {
                    suggestedToolNames = llmResponse.split(',').map(s => s.trim()).filter(Boolean);
                }
                const relevantTools = allTools.tools.filter((t) => suggestedToolNames.includes(t.name));
                if (relevantTools.length > 0) {
                    ...
                    console.error(`[LOG][CHAT] [TOOL_SELECTION] LLM selected ${relevantTools.length} tools: ${relevantTools.map((t) => t.name).join(', ')}`);
                }
                else {
        ...
        const toolsClaude = mcpTools.tools.map((tool) => ({
            name: tool.name,
        ...
        if (selectedModel === 'openai') {
            ...
            while (!finished && recursion < MAX_RECURSION) {
                ...
                if (response.type === "message" && Array.isArray(response.content)) {
                        ...
                        if (block.type === "text") {
                            ...
                            try {
                                ...
                                if (toolResult && Array.isArray(toolResult.content)) {
                                    toolResultContent = toolResult.content.map((c) => ({ type: 'text', text: c.text || JSON.stringify(c) }));
                                }
                                else {
                                ...
                                finalTexts.push(toolResultContent.map((c) => c.text).join('\n'));
                                finished = false;
                            }
                            catch (err) {
                    ...
                    finished = !response.content.some((c) => c.type === "tool_use");
                }
                else {
                    finished = true;
                }
            ...
            const toolCallTracker = new Set();
            try {
                while (!finished && recursion < MAX_RECURSION) {
                    console.error(`[LOG][ANTHROPIC] Chamada para Claude API (iteração ${recursion + 1}/${MAX_RECURSION})`);
                    apiResponseObject = await callClaudeDirectAPI(currentTurnMessages, anthropicTools, clientId, systemPrompt);
                    const assistantResponseContent = (apiResponseObject.content || [])
                        .map((block) => {
                        if (block.type === 'text')
                    ...
                    else if (apiResponseObject.stop_reason === 'end_turn' && !apiResponseObject.content?.some((b) => b.type === 'text')) {
                    ...
                    if (apiResponseObject.stop_reason === 'tool_use') {
                        ...
                        if (toolResultsForThisTurn.every(tr => tr.is_error && toolCallTracker.has(`${(toolUseBlocks.find(tu => tu.id === tr.tool_use_id) || {}).name}_${JSON.stringify((toolUseBlocks.find(tu => tu.id === tr.tool_use_id) || {}).input || {})}`))) {
...
})); // Cast to any to bypass the stubborn linter error for now
    ...
    try {
        ...
        if (enableContextFiltering && context) {
            filteredTools = allTools.filter(tool => {
                return tool.contexts.some((c) => context.toLowerCase().split(',').some(contextPart => c.toLowerCase().includes(contextPart.trim()) ||
        ...
        if (category) {
            filteredTools = filteredTools.filter(tool => {
                return tool.categories.some((c) => c.toLowerCase().includes(category.toLowerCase()) ||
...
app.post('/tools/metrics/reset', (req, res) => {

...
/**
 * Interface for tool filtering options
 */
export interface ToolFilterOptions {
  context?: string;    // The current context (e.g., "email", "calendar", "finance")
  category?: string;   // Tool category (e.g., "communication", "search", "calculation")
  userId?: string;     // User identifier for personalized tool access
  limit?: number;      // Max number of tools to return
}
 ...
 */
export async function fetchDynamicTools(options: ToolFilterOptions = {}, baseUrl: string = DEFAULT_MCP_URL) {
  try {
 ...
 */
export async function getToolMetrics(baseUrl: string = DEFAULT_MCP_URL) {
  try {
 ...
 * Resets tool metrics
 */
export async function resetToolMetrics(baseUrl: string = DEFAULT_MCP_URL) {
  try {
...
export class DynamicToolClient {
  ...
  private toolCache: Map<string, any> = new Map(); // Cache tools to avoid repeated requests
  
  constructor(client: Client, baseUrl: string = DEFAULT_MCP_URL) {
    this.client = client;
   ...
   */
  async getTools(options: ToolFilterOptions = {}) {

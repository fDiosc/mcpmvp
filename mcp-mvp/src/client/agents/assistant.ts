 ...
 */
export async function createDynamicAssistant(mcpClient: Client) {
  // Descobre as ferramentas MCP dinamicamente
  const tools = await mcpClient.listTools();
  const openaiTools = tools.tools.map((tool: any) => ({
    type: 'function',
    function: {
      name: tool.name,
 ...
 */
export async function createAssistant(tools: any[]) {
  try {
 ...
 */
export async function sendMessage(client: Client, threadId: string, assistantId: string, content: string) {
  try {
    ...
    if (assistantMessages.length > 0) {
      ...
      const textBlock = contentBlocks.find((block: any) => block.type === "text" && typeof block.text === "object" && typeof block.text.value === "string");
      if (textBlock) {
        console.log('Resposta do assistente (bloco de texto):', (textBlock as any).text.value);
        return (textBlock as any).text.value;
      }
 ...
 */
export async function createAssistantWithMcpServer(mcpServerUrl: string) {
  try {

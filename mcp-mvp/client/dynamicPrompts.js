 ...
 */
export async function detectPromptsFromMessage(message, mcpClient, selectedModel) {
    ...
    const promptDescriptions = availablePrompts.map(prompt => `- ${prompt.name}: ${prompt.description || `Prompt para ${prompt.name}`}`).join('\n');
...
export class DynamicPromptClient {
     ...
     */
    async getPromptFromMessage(message, selectedModel) {
     ...
     */
    formatMessagesWithPrompt(userMessage, promptResult, shouldAppend) {
        ...
        const systemMessages = promptResult.messages.filter((m) => m.role === 'system');
        const promptUserMessages = promptResult.messages.filter((m) => m.role === 'user');
        let messages = [];
        ...
        // Abordagem 2: Concatenar o texto do prompt com a mensagem do usuário
            ...
            const promptText = promptUserMessages.map((m) => {
     ...
     */
    getPromptContent(promptName, params = {}) {
 ...
 */
export function logPromptDetection(message, promptName, params = {}) {
